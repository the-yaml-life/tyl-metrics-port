...
use tyl_metrics_port::{
    MetricRequest, MetricsManager, MockMetricsAdapter, MockMetricsConfig, Labels, MetricType
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = MetricRequest::gauge("active_connections", 42.0)
    ...
    for duration_ms in [12, 45, 23, 67, 34, 89, 15] {
        let request = MetricRequest::histogram("request_duration_seconds", duration_ms as f64 / 1000.0)
    ...
    if !snapshot.is_empty() {
        ...
        for metric in snapshot.iter().take(3) {
            ...
            println!("     {} = {} {:?}", metric.name, value_str, metric.metric_type);
        }
    }
