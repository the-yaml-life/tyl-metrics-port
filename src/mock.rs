...
use crate::utils::{validate_metric_name, validate_labels, validate_counter_value, validate_metric_value};
...
mod tests {
    ...
    async fn test_find_metrics_by_name() {
        ...
        adapter.record(&MetricRequest::counter("http_requests", 1.0)).await.unwrap();
        adapter.record(&MetricRequest::gauge("memory_usage", 512.0)).await.unwrap();
        adapter.record(&MetricRequest::counter("http_requests", 2.0)).await.unwrap();
    ...
    async fn test_find_metrics_by_type() {
        ...
        adapter.record(&MetricRequest::counter("counter1", 1.0)).await.unwrap();
        adapter.record(&MetricRequest::counter("counter2", 2.0)).await.unwrap();
        adapter.record(&MetricRequest::gauge("gauge1", 100.0)).await.unwrap();
    ...
    async fn test_find_metrics_with_label() {
        ...
        adapter.record(&MetricRequest::counter("requests", 1.0)
            .with_label("method", "GET")).await.unwrap();
        adapter.record(&MetricRequest::counter("requests", 2.0)
            .with_label("method", "POST")).await.unwrap();
        adapter.record(&MetricRequest::counter("requests", 1.0)
            .with_label("method", "GET")).await.unwrap();
    ...
    async fn test_failure_simulation() {
        ...
        assert!(result.unwrap_err().to_string().contains("Simulated recording failure"));
    }
    ...
    async fn test_get_snapshot() {
        ...
        adapter.record(&MetricRequest::counter("test", 1.0)).await.unwrap();
